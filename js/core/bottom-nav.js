...
window.RKNav = (function () {
  ...
  // don't load theme-anime.css, so the rule can't live there. Falls back to
  // the --rk-* global tokens when a page doesn't define its own local
  // --card/--border/--text-low/--lavender (e.g. play.html).
  function _ensureStyle() {
    ...
    style.textContent =
      ...
      'background:var(--card,var(--rk-card,#fff));' +
      'border-top:1px solid var(--border,var(--rk-stroke,rgba(0,0,0,.08)));' +
      'padding:8px 4px calc(8px + env(safe-area-inset-bottom))}' +
      '.rk-nav-item{display:flex;flex-direction:column;align-items:center;gap:2px;text-decoration:none;' +
      'color:var(--text-low,var(--rk-muted-ui,#9a9ab0));font-size:10px;font-weight:700;padding:4px 10px;border-radius:10px}' +
      '.rk-nav-item.active{color:var(--lavender,#7c3aed)}' +
